...
public class FcmService extends FirebaseMessagingService {
    ...
    public static final String CUSTOM_NOTIFICATION_CHANNEL = "fcm_default_channel_beegleapp";
    ...
    public void onMessageReceived(@NonNull RemoteMessage remoteMessage) {
        ...
        if (remoteMessage.getData().size() > 0) {
            Log.d("DEBUG", "Payload: "+remoteMessage.getData().toString());
            if (!remoteMessage.getData().get("payload").isEmpty()) {
                crateNotification(remoteMessage);
            }
        } else {
            Log.d("DEBUG", "Data in payload empty");
        }
    }
